...
export function TeamSwitcher() {
  ...
  const [warehouse, setWarehouse] = React.useState<string | null>(selectedWarehouse);
          ...
          <DropdownMenu>
              ...
              <SidebarMenuButton size="lg" className="data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground">
                ...
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-semibold capitalize">
                    {selectedWarehouse ? warehouses.find(val => val.id === selectedWarehouse)?.name : warehouses[0].name}
                  </span>
                  <span className="truncate text-xs capitalize">
                    {selectedWarehouse ? warehouses.find(val => val.id === selectedWarehouse)?.location : warehouses[0].location}
                  </span>
                </div>
                <ChevronsUpDown className="ml-auto" />
              </SidebarMenuButton>
            ...
            >
              ...
              {warehouses.map(warehouse => (
                <DropdownMenuItem key={warehouse.name} onClick={() => setWarehouse(warehouse.id)} className="gap-2 p-2">
...
const DialogAddWarehouse = ({ isAdd, setIsAdd }: { isAdd: boolean; setIsAdd: React.Dispatch<React.SetStateAction<boolean>> }) => {
  const {
    register,
    handleSubmit,
    formState: { errors }
  ...
  const warehouseMutation = useMutation({
    mutationFn: async (data: Prisma.WarehouseCreateInput) => await addWarehouse(data),
    onSuccess: () => {
      toast({
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 py-4">
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="name" {...register('name')} className="col-span-3" placeholder="Example Warehouse" />
